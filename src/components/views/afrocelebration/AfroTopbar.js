  ...
  return (
    <div className=" py-2 px-4 flex items-center justify-between">
      ...
      <div className="flex space-x-2 items-center">
        <select className="bg-gray-100 text-gray-600 py-2 px-3 rounded-md border border-gray-400 focus:outline-none focus:border-black">
...

        {
            menuItems.map((item, index) => (
                <button key={index} className="bg-black text-white py-2 px-2 rounded-md">{item.name}</button>
            ))
        }
      </div>
    </div>
